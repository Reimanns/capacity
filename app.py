...
<details class="scenario" open>
  ...
  <div class="scenario-actions">
    ...
    <span style="font-size:12px;color:#6b7280;">Discipline: uses the <em>current</em> selection above; OT% increases capacity only for the comparison (doesn’t change the main chart).</span>
  </div>
...
function scenarioKPIs(opts){
  ...
  // Optional: project slip/penalty using What-If style computation (worst dept slip)
  ...
  if(!isNaN(idx) && idx >= 0 && idx < all.length){
    ...
    // Earliest allowable start based on lead
    ...
    // Headroom sum across period for each dept; take worst slip
    function capPerDay(keyDept){ const dept=departmentCapacities.find(x=>x.key===keyDept); const perWeek=(dept?.headcount||0)*HOURS_PER_FTE*PRODUCTIVITY_FACTOR; const uplift=1+Math.max(0,(parseFloat(opts.ot)||0))/100; return (perWeek*uplift)/5.0; }
    function periodRange(period, labels, start, end){
      let s=-1,e=-1;
      for(let i=0;i<labels.length;i++){
        const L=parseDateLocalISO(labels[i]);
...
// Recalc table if the user switches period/discipline so results stay relevant
sel.addEventListener('change', ()=>{ /* optional: auto re-run */ });
periodSel.addEventListener('change', ()=>{ /* optional: auto re-run */ });
</script>
